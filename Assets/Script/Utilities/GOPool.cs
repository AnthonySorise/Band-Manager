...
///       GOPool.Spawn(somePrefab, somePosition, someRotation);
/// 
///   Instead of destroying an object, use this:
///       GOPool.Despawn(myGameObject);
/// 
///   If desired, you can preload the pool with a number of instances:
///       GOPool.Preload(somePrefab, 20);
/// 
/// DOESN'T trigger Awake() or Start() for the GO being spawned.  You should reset your
...
public static class GOPool
{
    ...
    //     }
    // }
	//Modified by AS to allow for specifying the parent of preloaded GOs in Unity's hierarchy
    static public void Preload(GameObject prefab, Transform parent = null, int qty = 1)
    {
        Init(prefab, qty);
        ...
        for (int i = 0; i < qty; i++)
        {
            obs[i] = Spawn(prefab, Vector3.zero, Quaternion.identity);
            if(parent != null){
				obs[i].transform.SetParent(parent, true);
			}
        }
        // Now despawn them all.
        for (int i = 0; i < qty; i++)
